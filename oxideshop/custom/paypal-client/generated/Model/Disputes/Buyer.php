...
class Buyer implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->email) || Assert::minLength(
            $this->email,
            3,
            "email in Buyer must have minlength of 3 $within"
        );
        !isset($this->email) || Assert::maxLength(
            $this->email,
            254,
            "email in Buyer must have maxlength of 254 $within"
        );
        !isset($this->payer_id) || Assert::minLength(
            $this->payer_id,
            13,
            "payer_id in Buyer must have minlength of 13 $within"
        );
        !isset($this->payer_id) || Assert::maxLength(
            $this->payer_id,
            13,
            "payer_id in Buyer must have maxlength of 13 $within"
        );
        !isset($this->name) || Assert::minLength(
            $this->name,
            1,
            "name in Buyer must have minlength of 1 $within"
        );
        !isset($this->name) || Assert::maxLength(
            $this->name,
            2000,
            "name in Buyer must have maxlength of 2000 $within"
        );
    }
