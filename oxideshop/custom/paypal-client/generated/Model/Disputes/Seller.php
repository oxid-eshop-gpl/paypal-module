...
class Seller implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->email) || Assert::minLength(
            $this->email,
            3,
            "email in Seller must have minlength of 3 $within"
        );
        !isset($this->email) || Assert::maxLength(
            $this->email,
            254,
            "email in Seller must have maxlength of 254 $within"
        );
        !isset($this->merchant_id) || Assert::minLength(
            $this->merchant_id,
            1,
            "merchant_id in Seller must have minlength of 1 $within"
        );
        !isset($this->merchant_id) || Assert::maxLength(
            $this->merchant_id,
            255,
            "merchant_id in Seller must have maxlength of 255 $within"
        );
        !isset($this->name) || Assert::minLength(
            $this->name,
            1,
            "name in Seller must have minlength of 1 $within"
        );
        !isset($this->name) || Assert::maxLength(
            $this->name,
            2000,
            "name in Seller must have maxlength of 2000 $within"
        );
    }
