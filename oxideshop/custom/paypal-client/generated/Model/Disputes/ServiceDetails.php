...
class ServiceDetails implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->description) || Assert::minLength(
            $this->description,
            1,
            "description in ServiceDetails must have minlength of 1 $within"
        );
        !isset($this->description) || Assert::maxLength(
            $this->description,
            2000,
            "description in ServiceDetails must have maxlength of 2000 $within"
        );
        !isset($this->service_started) || Assert::minLength(
            $this->service_started,
            1,
            "service_started in ServiceDetails must have minlength of 1 $within"
        );
        !isset($this->service_started) || Assert::maxLength(
            $this->service_started,
            255,
            "service_started in ServiceDetails must have maxlength of 255 $within"
        );
        !isset($this->note) || Assert::minLength(
            $this->note,
            1,
            "note in ServiceDetails must have minlength of 1 $within"
        );
        !isset($this->note) || Assert::maxLength(
            $this->note,
            2000,
            "note in ServiceDetails must have maxlength of 2000 $within"
        );
        !isset($this->sub_reasons) || Assert::isArray(
            $this->sub_reasons,
            "sub_reasons in ServiceDetails must be array $within"
        );
        !isset($this->purchase_url) || Assert::minLength(
            $this->purchase_url,
            1,
            "purchase_url in ServiceDetails must have minlength of 1 $within"
        );
        !isset($this->purchase_url) || Assert::maxLength(
            $this->purchase_url,
            2000,
            "purchase_url in ServiceDetails must have maxlength of 2000 $within"
        );
    }
