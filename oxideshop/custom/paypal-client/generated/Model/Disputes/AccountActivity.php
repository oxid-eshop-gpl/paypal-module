...
class AccountActivity implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->id) || Assert::minLength(
            $this->id,
            1,
            "id in AccountActivity must have minlength of 1 $within"
        );
        !isset($this->id) || Assert::maxLength(
            $this->id,
            255,
            "id in AccountActivity must have maxlength of 255 $within"
        );
        !isset($this->create_time) || Assert::minLength(
            $this->create_time,
            20,
            "create_time in AccountActivity must have minlength of 20 $within"
        );
        !isset($this->create_time) || Assert::maxLength(
            $this->create_time,
            64,
            "create_time in AccountActivity must have maxlength of 64 $within"
        );
        !isset($this->entity_type) || Assert::minLength(
            $this->entity_type,
            1,
            "entity_type in AccountActivity must have minlength of 1 $within"
        );
        !isset($this->entity_type) || Assert::maxLength(
            $this->entity_type,
            255,
            "entity_type in AccountActivity must have maxlength of 255 $within"
        );
        !isset($this->entity_subtype) || Assert::minLength(
            $this->entity_subtype,
            1,
            "entity_subtype in AccountActivity must have minlength of 1 $within"
        );
        !isset($this->entity_subtype) || Assert::maxLength(
            $this->entity_subtype,
            255,
            "entity_subtype in AccountActivity must have maxlength of 255 $within"
        );
        !isset($this->action_performed) || Assert::minLength(
            $this->action_performed,
            1,
            "action_performed in AccountActivity must have minlength of 1 $within"
        );
        !isset($this->action_performed) || Assert::maxLength(
            $this->action_performed,
            255,
            "action_performed in AccountActivity must have maxlength of 255 $within"
        );
        !isset($this->entity_id) || Assert::minLength(
            $this->entity_id,
            1,
            "entity_id in AccountActivity must have minlength of 1 $within"
        );
        !isset($this->entity_id) || Assert::maxLength(
            $this->entity_id,
            255,
            "entity_id in AccountActivity must have maxlength of 255 $within"
        );
        !isset($this->activity_entity_info) || Assert::isInstanceOf(
            $this->activity_entity_info,
            ActivityEntityInfo::class,
            "activity_entity_info in AccountActivity must be instance of ActivityEntityInfo $within"
        );
