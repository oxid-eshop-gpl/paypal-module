...
class CommunicationDetails implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->email) || Assert::minLength(
            $this->email,
            3,
            "email in CommunicationDetails must have minlength of 3 $within"
        );
        !isset($this->email) || Assert::maxLength(
            $this->email,
            254,
            "email in CommunicationDetails must have maxlength of 254 $within"
        );
        !isset($this->note) || Assert::minLength(
            $this->note,
            1,
            "note in CommunicationDetails must have minlength of 1 $within"
        );
        !isset($this->note) || Assert::maxLength(
            $this->note,
            2000,
            "note in CommunicationDetails must have maxlength of 2000 $within"
        );
        !isset($this->time_posted) || Assert::minLength(
            $this->time_posted,
            20,
            "time_posted in CommunicationDetails must have minlength of 20 $within"
        );
        !isset($this->time_posted) || Assert::maxLength(
            $this->time_posted,
            64,
            "time_posted in CommunicationDetails must have maxlength of 64 $within"
        );
    }
