...
class Transaction implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->id) || Assert::minLength(
            $this->id,
            1,
            "id in Transaction must have minlength of 1 $within"
        );
        !isset($this->id) || Assert::maxLength(
            $this->id,
            255,
            "id in Transaction must have maxlength of 255 $within"
        );
        !isset($this->items) || Assert::isArray(
            $this->items,
            "items in Transaction must be array $within"
        );
        if (isset($this->items)) {
        ...
        !isset($this->status) || Assert::minLength(
            $this->status,
            1,
            "status in Transaction must have minlength of 1 $within"
        );
        !isset($this->status) || Assert::maxLength(
            $this->status,
            255,
            "status in Transaction must have maxlength of 255 $within"
        );
        ...
        !isset($this->create_time) || Assert::minLength(
            $this->create_time,
            20,
            "create_time in Transaction must have minlength of 20 $within"
        );
        !isset($this->create_time) || Assert::maxLength(
            $this->create_time,
            64,
            "create_time in Transaction must have maxlength of 64 $within"
        );
    }
