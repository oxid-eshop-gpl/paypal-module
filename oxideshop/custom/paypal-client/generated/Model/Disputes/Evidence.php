...
class Evidence implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->evidence_type) || Assert::minLength(
            $this->evidence_type,
            1,
            "evidence_type in Evidence must have minlength of 1 $within"
        );
        !isset($this->evidence_type) || Assert::maxLength(
            $this->evidence_type,
            255,
            "evidence_type in Evidence must have maxlength of 255 $within"
        );
        ...
        !isset($this->source) || Assert::minLength(
            $this->source,
            1,
            "source in Evidence must have minlength of 1 $within"
        );
        !isset($this->source) || Assert::maxLength(
            $this->source,
            255,
            "source in Evidence must have maxlength of 255 $within"
        );
        !isset($this->date) || Assert::minLength(
            $this->date,
            20,
            "date in Evidence must have minlength of 20 $within"
        );
        !isset($this->date) || Assert::maxLength(
            $this->date,
            64,
            "date in Evidence must have maxlength of 64 $within"
        );
        !isset($this->item_id) || Assert::minLength(
            $this->item_id,
            1,
            "item_id in Evidence must have minlength of 1 $within"
        );
        !isset($this->item_id) || Assert::maxLength(
            $this->item_id,
            255,
            "item_id in Evidence must have maxlength of 255 $within"
        );
    }
