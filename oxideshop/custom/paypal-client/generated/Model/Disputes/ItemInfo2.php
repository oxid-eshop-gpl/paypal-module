 ...
 */
class ItemInfo2 implements JsonSerializable
{
    use BaseModel;
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->id) || Assert::minLength(
            $this->id,
            1,
            "id in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->id) || Assert::maxLength(
            $this->id,
            255,
            "id in ItemInfo2 must have maxlength of 255 $within"
        );
        !isset($this->category) || Assert::minLength(
            $this->category,
            1,
            "category in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->category) || Assert::maxLength(
            $this->category,
            255,
            "category in ItemInfo2 must have maxlength of 255 $within"
        );
        !isset($this->description) || Assert::minLength(
            $this->description,
            1,
            "description in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->description) || Assert::maxLength(
            $this->description,
            2000,
            "description in ItemInfo2 must have maxlength of 2000 $within"
        );
        !isset($this->partner_transaction_id) || Assert::minLength(
            $this->partner_transaction_id,
            1,
            "partner_transaction_id in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->partner_transaction_id) || Assert::maxLength(
            $this->partner_transaction_id,
            255,
            "partner_transaction_id in ItemInfo2 must have maxlength of 255 $within"
        );
        !isset($this->reason) || Assert::minLength(
            $this->reason,
            1,
            "reason in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->reason) || Assert::maxLength(
            $this->reason,
            255,
            "reason in ItemInfo2 must have maxlength of 255 $within"
        );
        !isset($this->sub_reason) || Assert::minLength(
            $this->sub_reason,
            1,
            "sub_reason in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->sub_reason) || Assert::maxLength(
            $this->sub_reason,
            255,
            "sub_reason in ItemInfo2 must have maxlength of 255 $within"
        );
        !isset($this->amount) || Assert::isInstanceOf(
            $this->amount,
            CommonV3Money::class,
            "amount in ItemInfo2 must be instance of CommonV3Money $within"
        );
        !isset($this->amount) ||  $this->amount->validate(ItemInfo2::class);
        !isset($this->note) || Assert::minLength(
            $this->note,
            1,
            "note in ItemInfo2 must have minlength of 1 $within"
        );
        !isset($this->note) || Assert::maxLength(
            $this->note,
            2000,
            "note in ItemInfo2 must have maxlength of 2000 $within"
        );
    }
