...
class OfferHistory implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->offer_time) || Assert::minLength(
            $this->offer_time,
            20,
            "offer_time in OfferHistory must have minlength of 20 $within"
        );
        !isset($this->offer_time) || Assert::maxLength(
            $this->offer_time,
            64,
            "offer_time in OfferHistory must have maxlength of 64 $within"
        );
        !isset($this->actor) || Assert::minLength(
            $this->actor,
            1,
            "actor in OfferHistory must have minlength of 1 $within"
        );
        !isset($this->actor) || Assert::maxLength(
            $this->actor,
            255,
            "actor in OfferHistory must have maxlength of 255 $within"
        );
        !isset($this->event_type) || Assert::minLength(
            $this->event_type,
            1,
            "event_type in OfferHistory must have minlength of 1 $within"
        );
        !isset($this->event_type) || Assert::maxLength(
            $this->event_type,
            255,
            "event_type in OfferHistory must have maxlength of 255 $within"
        );
        !isset($this->offer_type) || Assert::minLength(
            $this->offer_type,
            1,
            "offer_type in OfferHistory must have minlength of 1 $within"
        );
        !isset($this->offer_type) || Assert::maxLength(
            $this->offer_type,
            255,
            "offer_type in OfferHistory must have maxlength of 255 $within"
        );
        !isset($this->offer_amount) || Assert::isInstanceOf(
            $this->offer_amount,
            CommonV3Money::class,
            "offer_amount in OfferHistory must be instance of CommonV3Money $within"
        );
