...
class AcceptClaim implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->note) || Assert::minLength(
            $this->note,
            1,
            "note in AcceptClaim must have minlength of 1 $within"
        );
        !isset($this->note) || Assert::maxLength(
            $this->note,
            2000,
            "note in AcceptClaim must have maxlength of 2000 $within"
        );
        !isset($this->accept_claim_reason) || Assert::minLength(
            $this->accept_claim_reason,
            1,
            "accept_claim_reason in AcceptClaim must have minlength of 1 $within"
        );
        !isset($this->accept_claim_reason) || Assert::maxLength(
            $this->accept_claim_reason,
            255,
            "accept_claim_reason in AcceptClaim must have maxlength of 255 $within"
        );
        !isset($this->invoice_id) || Assert::minLength(
            $this->invoice_id,
            1,
            "invoice_id in AcceptClaim must have minlength of 1 $within"
        );
        !isset($this->invoice_id) || Assert::maxLength(
            $this->invoice_id,
            127,
            "invoice_id in AcceptClaim must have maxlength of 127 $within"
        );
