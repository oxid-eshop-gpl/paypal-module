...
class Offer implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->offer_type) || Assert::minLength(
            $this->offer_type,
            1,
            "offer_type in Offer must have minlength of 1 $within"
        );
        !isset($this->offer_type) || Assert::maxLength(
            $this->offer_type,
            255,
            "offer_type in Offer must have maxlength of 255 $within"
        );
        !isset($this->history) || Assert::isArray(
            $this->history,
            "history in Offer must be array $within"
        );
    ...
    private function map(array $data)
    {
        ...
        if (isset($data['history'])) {
            ...
            foreach ($data['history'] as $item) {
                $this->history[] = new OfferHistory($item);
            }
        }
    }
