...
class PartnerAction implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        Assert::minLength(
            $this->id,
            1,
            "id in PartnerAction must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->id,
            255,
            "id in PartnerAction must have maxlength of 255 $within"
        );
        Assert::notNull($this->name, "name in PartnerAction must not be NULL $within");
        Assert::minLength(
            $this->name,
            1,
            "name in PartnerAction must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->name,
            255,
            "name in PartnerAction must have maxlength of 255 $within"
        );
        Assert::notNull($this->create_time, "create_time in PartnerAction must not be NULL $within");
        Assert::minLength(
            $this->create_time,
            20,
            "create_time in PartnerAction must have minlength of 20 $within"
        );
        Assert::maxLength(
            $this->create_time,
            64,
            "create_time in PartnerAction must have maxlength of 64 $within"
        );
        !isset($this->update_time) || Assert::minLength(
            $this->update_time,
            20,
            "update_time in PartnerAction must have minlength of 20 $within"
        );
        !isset($this->update_time) || Assert::maxLength(
            $this->update_time,
            64,
            "update_time in PartnerAction must have maxlength of 64 $within"
        );
        !isset($this->due_time) || Assert::minLength(
            $this->due_time,
            20,
            "due_time in PartnerAction must have minlength of 20 $within"
        );
        !isset($this->due_time) || Assert::maxLength(
            $this->due_time,
            64,
            "due_time in PartnerAction must have maxlength of 64 $within"
        );
        Assert::notNull($this->status, "status in PartnerAction must not be NULL $within");
        Assert::minLength(
            $this->status,
            1,
            "status in PartnerAction must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->status,
            255,
            "status in PartnerAction must have maxlength of 255 $within"
        );
