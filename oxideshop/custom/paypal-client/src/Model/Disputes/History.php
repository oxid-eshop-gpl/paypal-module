...
class History implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->date) || Assert::minLength(
            $this->date,
            20,
            "date in History must have minlength of 20 $within"
        );
        !isset($this->date) || Assert::maxLength(
            $this->date,
            64,
            "date in History must have maxlength of 64 $within"
        );
        !isset($this->actor) || Assert::minLength(
            $this->actor,
            1,
            "actor in History must have minlength of 1 $within"
        );
        !isset($this->actor) || Assert::maxLength(
            $this->actor,
            255,
            "actor in History must have maxlength of 255 $within"
        );
        !isset($this->event_type) || Assert::minLength(
            $this->event_type,
            1,
            "event_type in History must have minlength of 1 $within"
        );
        !isset($this->event_type) || Assert::maxLength(
            $this->event_type,
            255,
            "event_type in History must have maxlength of 255 $within"
        );
    }
