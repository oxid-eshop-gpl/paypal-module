...
class PartnerConfigOverride implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->partner_logo_url) || Assert::minLength(
            ...
            1,
            "partner_logo_url in PartnerConfigOverride must have minlength of 1 $within"
        );
        !isset($this->partner_logo_url) || Assert::maxLength(
            $this->partner_logo_url,
            127,
            "partner_logo_url in PartnerConfigOverride must have maxlength of 127 $within"
        );
        !isset($this->return_url) || Assert::minLength(
            $this->return_url,
            1,
            "return_url in PartnerConfigOverride must have minlength of 1 $within"
        );
        !isset($this->return_url) || Assert::maxLength(
            $this->return_url,
            127,
            "return_url in PartnerConfigOverride must have maxlength of 127 $within"
        );
        !isset($this->return_url_description) || Assert::minLength(
            $this->return_url_description,
            1,
            "return_url_description in PartnerConfigOverride must have minlength of 1 $within"
        );
        !isset($this->return_url_description) || Assert::maxLength(
            $this->return_url_description,
            127,
            "return_url_description in PartnerConfigOverride must have maxlength of 127 $within"
        );
        !isset($this->action_renewal_url) || Assert::minLength(
            $this->action_renewal_url,
            1,
            "action_renewal_url in PartnerConfigOverride must have minlength of 1 $within"
        );
        !isset($this->action_renewal_url) || Assert::maxLength(
            $this->action_renewal_url,
            127,
            "action_renewal_url in PartnerConfigOverride must have maxlength of 127 $within"
        );
    }
